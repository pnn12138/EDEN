...
- **隐藏结局（3 条）**：加百列 `escape_eden`、米迦勒 `michael_slay`、路西法 `lucifer_awaken`

### 园中印记

第一章当前共 **29 枚园中印记**：探索 6、交互 9、玩法 7、结局 7。三条隐藏结局会写入跨局 `triggeredEndingIds`，但不计入普通结局集合；未解锁的隐藏印记不会提前显示标题和描述。
...
- Chapter 1 三位天使隐藏结局已接入四张 1920×1080 过场图，以及一张 512×512 的米迦勒隐藏结局印记图；路西法过场会在第 4 段切换第二张镜头。
