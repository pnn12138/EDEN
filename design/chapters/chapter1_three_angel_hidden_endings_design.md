...
- 每条结局使用独占的 1920×1080 PNG 过场图，不复用普通成功或失败结局背景。
...
| 色调 | 青绿、冷蓝、深紫、暖白晨星点光 |
| 专属图片 | `public/assets/chapter1/images/lucifer_awaken_ending.png` |
...
- 按用户明确要求，以《黑客帝国》中“虚拟世界之外存在承载意识的容器”这一缸中之脑设定作为叙事概念参照；不以电影具体美术造型为参考，不复制演员、服装、机械结构、后脑插口、绿色代码雨、Logo 或镜头。
- 视觉主体改为 EDEN 原生的巨型透明“种子观测容器”：外形同时像种子、果实与水滴，结构由第五道河流和树根网络编织，不使用工业人体舱外壳。
- 容器内是青绿色发光生命介质与少量冷凝水痕，以超现实水体而非医学营养液表现。
- 舱内不是现实玩家、女人或路西法，而是玩家在内层伊甸使用的“蛇形观测代理”：由水流、树根状神经纤维和光构成的非人体轮廓。
- 细线是树根与河网延伸出的发光纤维，连接容器外壁与蛇形光体周围，不使用可识别的人类后脑插管造型。
- 黑暗远景只出现更多空的种子状观测容器，不排列人体舱，暗示伊甸不是唯一观测单元。
- 舱壁映出第五道水流和破碎的伊甸全息倒影，一粒暖白晨星光屑保留路西法识别点。
- 不做血腥人体实验，不让科幻元素完全吞没 EDEN 的水、蛇、树根和晨星意象。
- 这是外层视觉隐喻，不解释蛇在内层神话中的来历；现实玩家仍是操作者，女人仍是主要 AI 智能体。该隐藏结局揭示的是“蛇形输入代理被观测”，不是重写玩家身份。
...
4. 你看见了：没有园子，没有河。只有一只巨大的观测容器，和其中由水光与树根细线维系的蛇形影子——那是你在园中借用的形体。
...
5. 重新开始时重置本地过场进度。
6. 图片 `onError` 后隐藏破图节点，保留对应 tone 的 CSS 背景和全部文案。
7. 尊重 `prefers-reduced-motion`，减少淡入淡出但不删除内容。

三条过场至少各有一个完整全屏场景。本轮不扩展为多张 CG，以控制比赛 Demo 体积和生成成本。
...
3. 你醒来的地方没有名字，也没有神话，只有一只仍在运行的观测容器。
...
- `lucifer_awaken`，type=`special`，解锁后标题“缸中之醒”，描述：“晨星碎片照亮第五道倒影。你看见伊甸只是被观测的园子，也看见了自己在其中借用的蛇形代理。”
...
- `doc/` 文件无删除、重命名或移动。
- 不覆盖用户现有未提交改动。
- `doc/AI_ASSET_RECORD.md` 追加三张过场图和一枚印记图标的生成工具、用途、最终路径和提示词摘要。
...
- 旧存档兼容，结局状态可保存与恢复。
- 图鉴、印记、复盘和跨局追踪一致。
- 三张过场图与米迦勒印记图标可正常加载并已记录 AI 资产来源。
...
- 三张专属过场图形成明确的 AI 视觉创作成果，可直接进入 Demo 视频、PPT 和 AI 创作说明。
