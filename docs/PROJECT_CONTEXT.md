...
Updated by: `CodeBuddy (Agent 展示优化 P1)`
Current phase: `Agent 架构升级 P1 展示优化：玩家界面展示记忆检索/信念变化/Skills觉醒/工具链/结局认知轨迹`
Current build status: `lint/typecheck/build pass on 2026-06-18; Agent 机制从后端能力转为玩家可见展示点`

一句话项目说明：

> EDEN 是一个浏览器端 AI 叙事游戏 Demo，玩家在 Chapter 0 扮演蛇，通过低语影响夏娃。Agent 架构升级后，Chapter 0 从"对话推进进度"升级为"玩家低语 -> Agent 记忆检索 -> 信念变化 -> Skills 觉醒 -> 工具调用 -> 结局"的认知博弈机制。四轴信念状态（curiosity / obedience / trustInSerpent / selfJudgement）取代单一 temptationProgress 作为核心认知模型，本地记忆碎片检索（RAG 游戏化）为 Agent 提供上下文，5 种认知能力（Skills）通过信念和记忆触发解锁，工具链从单一 eat_fruit 扩展为 look_at_tree / approach_tree / touch_fruit / eat_fruit / ask_about_death 行为链，刺猬作为环境反馈 Agent 根据世界状态改变行为。规则层仍是状态变化和工具执行的唯一权威，LLM 只能输出意图。

当前最重要目标：

1. 平台范围已收敛为桌面浏览器：移动端不再作为当前或后续开发目标，也不作为 Codex/CodeBuddy 验收项。
2. Chapter 0 对白/语音/结局优化已完成：反馈不再混入对话流、夏娃 Prompt 添加自然对白约束和 few-shot 示例、语音从开关改为多模式音色下拉、成功结局补完整上帝降临与逐出伊甸园叙事。
